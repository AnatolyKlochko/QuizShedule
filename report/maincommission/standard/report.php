...
/**
 * This file defines the quiz customresult report class.
 *
...
require_once($CFG->dirroot . '/mod/quiz/report/student/student_options.php');
require_once($CFG->dirroot . '/mod/quiz/report/student/student_form.php');
require_once($CFG->dirroot . '/mod/quiz/report/student/student_table.php');

//* TESTING-PARAMETERS */ $_POST['date_start'] = '\'"ab2016-05-10'; $_POST['date_finish'] = '2019-05-30'; $_POST['for_students'] = 'on'; $_POST['submit'] = 'submit';
//file_put_contents(__DIR__ . '/debug.txt', var_dump( $_POST ));

/**
 * Quiz report subclass for the ... report.
 *
 * @copyright 1999 onwards Martin Dougiamas and others {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_student_report extends quiz_attempts_report {

    public function display($quiz, $cm, $course) {
        global $DB, $OUTPUT, $PAGE, $USER;
        
        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'overview', 'quiz_student_settings_form', $quiz, $cm, $course);

        $options = new quiz_student_options('overview', $quiz, $cm, $course);
        ...
        // Displays all users, who have attempts. If quiz_attempts_report::ENROLLED_WITH, the report is empty.
        ...
        $table = new quiz_student_table($quiz, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $filename = quiz_report_download_filename(get_string('overviewfilename', 'quiz_student'),
...
//            }
            ...
            $this->add_reporttype_column($columns, $headers);
            $this->add_reportformat_column($columns, $headers);
            $this->add_actionbutton_column($columns, $headers);
...
//                $columns[] = 'regraded';
//                $headers[] = get_string('regrade', 'quiz_student');
//            }
...
//            }
            ...
            //$x = $table->sql;
            //echo '<div>123</div>';
            list( $is_admin, $is_lc, $is_employee ) = $this->get_user_groups( $course, $USER );
            if ($is_lc) {
                $this->show_lc_form();
            }
            if ($is_employee) {
                $this->show_employee_form();
            }
            
            // Share SQL
            $table->sql->where = 'quiza.id IS NOT NULL ';
            
            // Query is sent
            if( $this->is_submitted() ) {
                $table->sql->where .= ' AND ' . $this->get_date_where( );
                if ( $is_lc ) {
                    if ( ! $this->for_students() ) {
                        $table->sql->where .= ' AND u.id = ' . $USER->id;
                    }
                }
                if ( $is_employee ) {
                    $table->sql->where .= ' AND u.id = ' . $USER->id;
...
//        }
        ...
        // Add a js code
        $PAGE->requires->js( new moodle_url($CFG->wwwroot . '/mod/quiz/report/student/script.js') );
        
        return true;
    }
    
    private function get_user_groups( $course, $user ) {
    ...
    private function show_lc_form() {
        include __DIR__ . '/lc_form.php';
    }
    private function show_employee_form() {
        include './employee_form.php';
    }
    private function is_submitted() {
        return $_POST['submit'];
    }
    private function for_students( ) {
        return $_POST['for_students'] === 'on';
    }
    private function get_date_where() {
        $date_where = '';
        if ( isset( $_POST['date_start'] ) && isset( $_POST['date_finish'] ) ) {
            $ds = preg_replace('/[^0-9\.\-]/', '', $_POST['date_start']); //str_replace("'", '', $_POST['date_start']);DATE_ADD('2010-05-11', INTERVAL 1 DAY)
            $df = preg_replace('/[^0-9\.\-]/', '', $_POST['date_finish']);
            $date_where = 'quiza.timefinish >= UNIX_TIMESTAMP(\'' . $ds . '\') AND quiza.timefinish <= UNIX_TIMESTAMP(DATE_ADD(\'' . $df .'\', INTERVAL 1 DAY))';
            return $date_where;
        }
        return '';
    }

    protected function add_reporttype_column(&$columns, &$headers) {
        $columns[] = 'reporttype';
        $headers[] = get_string('reporttype', 'quiz_student');
    }
    protected function add_reportformat_column(&$columns, &$headers) {
        $columns[] = 'reportformat';
        $headers[] = get_string('reportformat', 'quiz_student');
    }
    protected function add_actionbutton_column(&$columns, &$headers) {
        $columns[] = 'actionbutton';
        $headers[] = get_string('actionbutton', 'quiz_student');
    }
