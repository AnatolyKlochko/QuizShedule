...
class renderer {
    ...
    private static function coltagattr_missedexam( $column_value ) {
        ...
        // Nearest Schedule
        $nearestpoint = self::coltagattr_missedexam_quizpointobj( $column_value['next'], $column_value['object'] );
        ...
        if ( $column_value['list'] ) {
            foreach ( $column_value['list'] as $point ) {
                array_unshift( $points, self::coltagattr_missedexam_quizpointobj( $point->datenextquiz, $point ) );
            }
        }
    ...
    private static function coltagattr_missedexam_quizpointobj( $nextdate, $schedulepoint ) {
        
        $quizpoint = new \stdClass();
        
        // Compute days count before or after Quiz
        list( $quizpoint->daysleft, $quizpoint->daysword ) = self::coltagattr_missedexam_quizpointarr( $nextdate );
        
        $quizpoint->quiz = $schedulepoint->quizname;
        
        // Moodle ID & report link
        $quizpoint->attemptobj = self::coltagattr_missedexam_attemptobj( $schedulepoint );
        
        // Check if current user is LC or RP, and give it ability change Schedule
        // data or print reports
        $quizpoint->toolboxobj = self::coltagattr_missedexam_toolboxobj( $schedulepoint );
    ...
    private static function coltagattr_missedexam_attemptobj( $schedulepoint ) {
        global $DB, $USER;
                
        $attemptobj = new \stdClass;
        
        if ( $USER->id == $schedulepoint->employeeid || ac_helper::is_lc( $USER->id ) || ac_helper::is_rp( $USER->id ) || ac_helper::is_moddev( $USER->id ) ) {
            
            $attemptid = ( new schedule )->attempt_get( $schedulepoint->employeeid, $schedulepoint->quizid );
        
            if ( $attemptid ) {

                $attemptobj->employeeid = $schedulepoint->employeeid;

                $attemptobj->quizid = $schedulepoint->quizid;

                $attemptobj->action = new \moodle_url( '/mod/quiz/report/student/report/result/report.php' );

                $timefinished = $DB->get_field( 'quiz_attempts', 'timefinish', [ 'id' => $attemptid ] );

                $attemptobj->timefinished = date( 'Y-m-d', $timefinished ) == date( 'Y-m-d', time() ) ? 'сьогодні' : date( 'H:i d-m-Y', $timefinished );

                $attemptobj->attemptid = $attemptid;
    ...
    private static function coltagattr_missedexam_attempthtml( $attemptobj ) {
                
        if ( isset( $attemptobj->action ) ) {
            
            $html = ' 
                <div class="moodle-wrapper">
                    <div class="id-wrapper">
                        <form class="report-frm" id="" target="_blank" method="post" action="'.$attemptobj->action.'">
                            <input type="hidden" name="id" value="'.$attemptobj->quizid.'">
                            <input type="hidden" name="userid" value="'.$attemptobj->employeeid.'">
                            <input type="hidden" name="attempt" value="'.$attemptobj->attemptid.'">
                                <span class="id">
                                    <span class="report-moodleid">Moodle ID&nbsp<button type="submit" class="report-link">'.$attemptobj->attemptid.'</button>,</span> завершено <span class="report-moodlepassdate">'.$attemptobj->timefinished.'</span>
                                </span>
                        </form>
                    </div>
                </div>';
    ...
    private static function coltagattr_missedexam_toolboxobj( $schedulepoint ) {
        global $USER;
        ...
        $obj->editlink = "$url?employeeid=$schedulepoint->employeeid&quizid=$schedulepoint->quizid&scheduleid=$schedulepoint->id";
        
        
        return $obj;
        
    }
    private static function coltagattr_missedexam_toolboxhtlm( $toolboxobj ) {
                      
        $html = ' 
            <div class="toolbox-wrapper">
                <div class="tool-item"><a href="'.$toolboxobj->editlink.'" target="_blank"><i class="icon fa fa-edit fa-fw"></i></a></div>
            </div>
        ';
    ...
    private static function coltagattr_missedexam_schedulehtml( $points ) {
        
        $html = '';
        
        $attemptcolumn_isset = false;
        
        foreach ( $points as $point ) {
            
            if ( isset( $point->attemptobj->attemptid ) ) {
                
                $attemptcolumn_isset = true;
        ...
        foreach ( $points as $point ) {
            ...
            '<div class="schedulepoint-wrapper">
                <div class="days-wrapper">
                    <span class="left">'.$point->daysleft.'</span>&nbsp<span class="word">'.$point->daysword.'</span>
                </div>
                <div class="quiz-name-wrapper">
                    <span class="title">'.$point->quiz.'</span>
                </div>' .
                ( ( $attemptcolumn_isset ) ? self::coltagattr_missedexam_attempthtml( $point->attemptobj ) : '' ) .
                ( ( $point->toolboxobj ) ? self::coltagattr_missedexam_toolboxhtlm( $point->toolboxobj ) : '' ) . '
            </div>';
    ...
    /**
     * Header Column 'Department'.
     */
    private function output_header_col_department( $row, $column) {
    ...
    private function output_header_col_safe_operation_of_lifts( $row, $column) {
        ...
        if ( $row === 'upper' ) {
            ...
            echo
                '<'.$tag.' rowspan="" colspan="3" class="'.$class.' rh-col-'.$column->key.'">' . 
                    '<'.$wrap.' class="'.$wrapclass.'">'.
                        '<div class="upper">'.
                            get_string( 'report_hcol_'.$column->key, 'quizschedule' ).
                        '</div>' . 
    ...
    /**
     * Header Column 'Pressure Vessels'.
     */
    private function output_header_col_pressure_vessels( $row, $column) {
     ...
     * Column 'Department'.
     */
    private function output_col_department( &$row, &$column ) {
...
//        }
     ...
     */
    private function output_col_pressure_vessels( &$row, &$column ) {
