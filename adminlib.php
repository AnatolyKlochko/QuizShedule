 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizschedule_admin_page_manage_quizschedule_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/quizschedule/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'quizschedule'),
                            $url);
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizschedule_plugin_manager {
     ...
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/quizschedule/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'quizschedule'),
        ...
        $shortsubtype = substr($this->subtype, strlen('quizschedule'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/quizschedule/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'quizschedule'));
    }
